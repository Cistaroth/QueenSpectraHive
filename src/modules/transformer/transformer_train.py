...
class TransformerTrainModule(TrainerBase):
    ...
    def __init__(
        ...
        seed: int = 42,
        save_path: Path = Path(__file__).parents[3] / "trained_models" / "transformer",
    ) -> None:
        """
        Initialize the step.
        ...
        Args:
            ...
            save_path (Path): Path to save the fine-tuned model. Defaults to the default save path.
    ...
    def _evaluate(
        ...
        Returns:
            tuple[float, float, float, float, float, float]: Average loss, accuracy, and the
                counts of predicted zeros, predicted ones, actual zeros and actual ones.
        """
        model.eval()
        running_loss = 0.0
        correct = 0
    ...
    def run(
        ...
        if verbose:
            console.section(title="Pretrained Audio Spectrogram Transformer")
