...
class CompositeModelModule(TrainerBase):
    ...
    def run(self, x_train: pd.DataFrame,y_train:pd.Series, verbose: bool = True) -> dict[str, torch.Tensor]:

        if verbose:
            console.section(title="Generating Composite model")
            logger.info(f"Processing on: {x_train.shape[-1] - 1} columns")

        dataset = BeeAudioDataset(df=x_train, labels=y_train, audio_dir=self._audio_dir, n_mfcc=self._n_mfcc)

        loader = DataLoader(dataset,batch_size=self._batch_size, shuffle=True, num_workers=self._num_workers, pin_memory=(self._device.type == "cuda"))

        x_train = x_train.drop(self._drop_column, axis=1)

        model = CompositeModel(
            self._lstm_layers, self._n_mfcc, ff_hidden_size=self._classification_hidden_size, layers=self._layers
...
class CompositeModel(
    ...
    def __init__(self, lstm_layers: tuple, cur_input_size, ff_hidden_size, layers):
...
class BeeAudioDataset(LazyAudioDataset):
    ...
    def __getitem__(self, idx: int):
        ...
        try:
            ...
            encoded_features = self._feature_extractor.run(waveform, sample_rate=self.TARGET_SR)
