...
async def spectral_inference(
    ...
    hive_temp: float | None = Form(None, description="Hive temperature (°C)"),
    hive_humidity: float | None = Form(None, description="Hive relative humidity (%)"),
    ...
    hive_number: int | None = Form(None, description="Hive number (1-5)"),
) -> InferenceResponseModel:
    
    content_type_check = file.content_type in {"audio/wav", "audio/x-wav", "audio/wave", "audio/vnd.wave"}

    filename_check = (file.filename or "").lower().endswith(".wav")
